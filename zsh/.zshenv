...
export_brew_package() {
    local package="$1"
    local sub_path="${2:-bin}"  # Default to 'bin' if sub_path not provided
    local package_path="/opt/homebrew/opt/${package}"
    local full_path="${package_path}/${sub_path}"

    # Check if the path exists before adding it
    if [[ -d "$full_path" ]]; then
        export PATH="${full_path}:${PATH}"
    else
        echo "Warning: Path '${full_path}' does not exist" >&2
    fi
}
...
# Legacy to import Python libs, maintain two versions 3.9 & 3.10, `pip3 list` & `pip3.9 list`
